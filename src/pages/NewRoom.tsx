...
export function NewRoom () {
    ...
    async function handleCreateRoom(event: FormEvent) {
        ...
        if (newRoom.trim() === '') { //conferindo se o nome da sala não está vazia
            return;
        }
        
        const roomRef = database.ref('rooms');

        const firebaseRoom = await roomRef.push({ //push usa quando é pra pegar uma informacao de uma lista
            title: newRoom,
            authorId: user?.id,
        })
